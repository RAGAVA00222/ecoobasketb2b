        ...
        <svg viewBox="0 0 320 380" className="h-auto w-full" role="img" aria-label="Tamil Nadu coverage — Chennai live, other zones coming soon">
          ...
          {zones.map((z, i) => (
            <g key={z.id}>
              {z.live && !reduce && (
                <motion.circle
                  cx={z.cx} cy={z.cy} r="6" fill="#16A34A"
                  initial={{ r: 6, opacity: 0.6 }}
                  animate={{ r: 18, opacity: 0 }}
                  transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }}
                />
              )}
              <circle cx={z.cx} cy={z.cy} r="5" fill={z.live ? "#0F5132" : "#FFFFFF"} stroke={z.live ? "#0F5132" : "#9CA3AF"} strokeWidth="1.5" />
              <text x={z.cx + 10} y={z.cy + 4} className="font-mono" fontSize="10" fill={z.live ? "#111827" : "#4B5563"}>
                {z.label}{z.live ? "" : " · soon"}
              </text>
            </g>
          ))}
